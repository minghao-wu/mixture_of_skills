...
class MultiDataset:
    ...
    def update_sampling_probabilities(self, new_probabilities, step, new_rewards=None):
        ...
        self.category_occurrences = OrderedDict([(k, 0) for k, v in self.datasets.items()])
        self.last_step = step
        self.write_one_log(step, self.category_probabilities)

        print("***** Update Sampling Probabilities Done *****")

    def write_one_log(self, step, probabilities):

        if self.prob_log_path is not None:
...
# paths = glob.glob("/home/minghaow/balancesft-project/datasets_mini/*.jsonl")
# raw_datasets = {}
# for p in paths:
#     print(f"Loading {p}")
...
# init_probs = {k: len(v) / sum([len(v) for v in raw_datasets.values()]) for k, v in raw_datasets.items()}
# rebalanced_probs = balance_probabilities_by_temperature(init_probs, 1)
# train_dataset = MultiDataset(raw_datasets, rebalanced_probs, tokenizer)

# print(train_dataset[0])
# print(train_dataset.sample_batch(4))

# out = model(**train_dataset.sample_batch(4))
# print(out)